                  ...
                  <table class="table table-hover text-nowrap">
                    ...
                    <tbody>
                      ...
                      @foreach($citas as $cita)
                        <tr>
                        <td title="{{$cita->obra}}">{{strlen($cita->obra) > 30 ? substr($cita->obra,0,29) : $cita->obra}}</td>
                        <td>{{$cita->tipo}}</td>
                        ...
                        @if($cita->confirmacion==1)
                          <a href="boleta/{{$cita->id}}" target="_blank" class="btn btn-info btn-sm d-inline p-2" title="Descargar boleta"><i class="fa fa-download" aria-hidden="true"></i> Boleta</a>
                        @endif
                        
                        </td>
                        <td>
                        @if($cita->confirmacion==3 || $cita->confirmacion==1)
                          @if($cita->esalcaldia==0) 
                          ...
                          @endif
                          @if($cita->esalcaldia==1) 
                          <!--Manifiesto siempre visible si no es falta-->
                          <a href="manifiestoalcaldia/{{$cita->id}}" target="_blank" class="btn btn-info btn-sm d-inline p-2" title="Descargar manifiesto"><i class="fa fa-download" aria-hidden="true"></i> Manifiesto</a>
                          @endif
                        ...
                        <td>
                        @if($cita->confirmacion==0)
                          <a href="firma/{{$cita->id}}" class="btn btn-info btn-sm d-inline p-2" title="Firma de recepción de material por parte del transportista."><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Entregar </a>
                        </th>
                        <th>
                          <a href="EntregaQr/{{$cita->id}}" class="btn btn-info btn-sm d-inline p-2" title="Entregar con qr al transportista."><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Entregar QR </a>
                        @endif
                        </td>
